...
sealed interface GraderRule {
   ...
   *
   * For example, if an album is 40 minutes long and the play record indicates that 30 minutes was
   * played, then this would return 0.75.
   */
   ...
   * @param minDurationPlayed The floor for whether or not we even consider giving this play through
   *   a grade using this rule. This is important because compactness is based on the expected
   *   runtime of the actual tracks played, not the expected runtime for the entire album. For
   *   example, if 50% of the album was played through (using duration as the measurement), but
