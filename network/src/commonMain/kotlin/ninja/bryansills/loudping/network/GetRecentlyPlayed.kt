...
class RealGetRecentlyPlayed(private val spotifyService: SpotifyService) : GetRecentlyPlayed {
  override fun invoke(
    startAt: Instant,
    stopAt: Instant,
    ...
    while (
      currentCoroutineContext().isActive &&
        keepGoing(currentQueryTime, stopAt, nextUrl, isFirstQuery)
    ) {
      val response =
        if (nextUrl != null) {
...
private fun List<PlayHistoryItem>.trim(olderThan: Instant): List<PlayHistoryItem> =
  this.filter { it.played_at > olderThan }
