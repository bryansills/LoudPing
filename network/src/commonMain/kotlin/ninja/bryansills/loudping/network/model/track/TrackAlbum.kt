...
data class TrackAlbum(
    ...
    @Serializable
    data class AlbumImage(val url: String, val height: Int? = -1, val width: Int? = -1)
}
...
sealed class AlbumType {
    @Serializable data object Album : AlbumType()

    @Serializable data object Single : AlbumType()

    @Serializable data object Compilation : AlbumType()

    @Serializable data class Unknown(val rawString: String) : AlbumType()
...
object AlbumTypeSerializer : KSerializer<AlbumType> {
    override val descriptor: SerialDescriptor =
        PrimitiveSerialDescriptor("AlbumType", PrimitiveKind.STRING)
...
sealed class ReleaseDatePrecision {
    @Serializable data object Year : ReleaseDatePrecision()

    @Serializable data object Month : ReleaseDatePrecision()

    @Serializable data object Day : ReleaseDatePrecision()

    @Serializable data class Unknown(val rawString: String) : ReleaseDatePrecision()
...
object ReleaseDatePrecisionSerializer : KSerializer<ReleaseDatePrecision> {
    override val descriptor: SerialDescriptor =
        PrimitiveSerialDescriptor("ReleaseDatePrecision", PrimitiveKind.STRING)
