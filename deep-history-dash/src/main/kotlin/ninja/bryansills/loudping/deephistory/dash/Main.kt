...
fun main() {
  runMosaicBlocking(onNonInteractive = NonInteractivePolicy.Ignore) {
        ...
        runStats =
          when (event) {
            ...
            is DeepHistoryRunEvent.RecordedChunk -> {
              runStats.copy(recordedCount = runStats.recordedCount + event.recorded.size)
            }
          }
      }
    }
    ...
    Column(modifier = Modifier.background(Color.Black)) {
      ...
      Text(value = "Total tracks: ${runStats.loadedCount}", color = Color.White)
      ...
      Text(value = "Recorded tracks: ${runStats.recordedCount}", color = Color.White)
    }
  }
}
