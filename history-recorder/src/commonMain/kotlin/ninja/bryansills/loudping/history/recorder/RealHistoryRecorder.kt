...
private fun List<RecentlyPlayedResponse>.toRecords(): List<PlayHistoryItem> =
  this.flatMap { networkRecord -> networkRecord.items }.sortedBy { it.played_at }

private fun List<PlayHistoryItem>.toDatabase(stopAt: Instant?): List<TrackPlayRecord> =
  this.filter { it.played_at > (stopAt ?: Instant.DISTANT_PAST) }
    .map { networkTrack ->
      TrackPlayRecord(
        track =
          Track(
...
private fun List<SimplifiedArtist>.toDatabase(): List<Artist> =
  this.map { networkArtist -> Artist(spotifyId = networkArtist.id, name = networkArtist.name) }
