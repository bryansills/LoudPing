...
class DefaultTrackAlbumGrouperTest {
  ...
  fun `simplest example`() = runTestTurbine {
    val album1 =
      (1..10).map { testRecord(index = it, albumId = "ALBUM001", timestamp = timestampTicker.next) }
    val album2 =
      (1..8).map { testRecord(index = it, albumId = "ignored", timestamp = timestampTicker.next) }
  ...
  fun `does not emit the last album`() = runTestTurbine {
    val album1 =
      (1..10).map { testRecord(index = it, albumId = "ALBUM001", timestamp = timestampTicker.next) }
    val album2 =
      (1..9).map { testRecord(index = it, albumId = "ALBUM002", timestamp = timestampTicker.next) }
    val album3 =
      (1..20).map {
        testRecord(index = it, albumId = "not-emitted", timestamp = timestampTicker.next)
      }
  ...
  fun `single album can be spread over 2 emissions`() = runTestTurbine {
    ...
    val album2chunk2 =
      (10..19).map {
        testRecord(index = it, albumId = "ALBUM002", timestamp = timestampTicker.next)
      }
    val album3 =
      (1..20).map {
        testRecord(index = it, albumId = "not-emitted", timestamp = timestampTicker.next)
  ...
  fun `can be single track albums`() = runTestTurbine {
    val singleTrackAlbums =
      (1..8).map { testRecord(index = 1, albumId = "ALBUM00$it", timestamp = timestampTicker.next) }

    val input = flow { singleTrackAlbums.forEach { emit(listOf(it)) } }
  ...
  fun `keeps track about what was played before`() = runTestTurbine {
    ...
    val album2 =
      (1..9).map { testRecord(index = it, albumId = "ALBUM002", timestamp = timestampTicker.next) }
    val album3 =
      (1..20).map { testRecord(index = it, albumId = "ALBUM003", timestamp = timestampTicker.next) }
    val album4 =
      (1..20).map {
        testRecord(index = it, albumId = "not-emitted", timestamp = timestampTicker.next)
      }
    ...
    val turbine =
      DefaultAlbumGrouper(previousTrackCount = previousTrackCount)
        .invoke(input)
  ...
  fun `keeps track about what was played after`() = runTestTurbine {
    ...
    val turbine =
      DefaultAlbumGrouper(followingTrackCount = followingTrackCount)
        .invoke(input)
        .testIn(backgroundScope)
