...
/**
 * Essentially copied from [Readability](https://github.com/mozilla/readability)
 *
 * There are 3 things I had to make changes in order to make the plain Readability library work with
 * HtmlUnit here:
 * - The spread operator doesn't seem to work??? Find the code `console.log(...args)` that I just
 *   commented out
 * - Regex is angry about some global options??? Find the regexes that I altered `adWords` and
 *   `loadingWords`
 * - The last 4 lines (starting with `(function() {`...) so that this script returns a result back
 *   to the Kotlin side
 */
internal val ReadabilityJs =
  ...
  |        // TKTK hey bryan here. console.log() is angry???
  |        // console.log(...args);
  ...
  |    return JSON.stringify(new Readability(documentClone).parse());
  |})();
  """
    .trimMargin()
