...
android {
    ...
    defaultConfig {
        ...
        val appVersionName = rootLocalProperties.getSecret("version.name", fallback = "0.0.69")
        val appVersionParts = appVersionName.split(".").map { it.toInt() }
        val appVersionCode =
            (10_000 * appVersionParts[0]) + (100 * appVersionParts[1]) + appVersionParts[2]

        versionCode = appVersionCode
        versionName = appVersionName

        val rootSecrets = rootProject.rootProperties("secrets.properties")
        buildConfigString("SneakSalt", rootSecrets.getSecret("sneak.salt"))
        buildConfigString("SneakClientId", rootSecrets.getSecret("sneak.clientid"))
        buildConfigString("SneakClientSecret", rootSecrets.getSecret("sneak.clientsecret"))
        buildConfigString("SneakRedirectUrl", rootSecrets.getSecret("sneak.redirecturl"))
        buildConfigString("SneakBaseApiUrl", rootSecrets.getSecret("sneak.baseapiurl"))
        buildConfigString("SneakBaseAuthApiUrl", rootSecrets.getSecret("sneak.baseauthapiurl"))
        buildConfigString("SneakAuthorizeUrl", rootSecrets.getSecret("sneak.authorizeurl"))
