...
class RealAlbumRepository(
  ...
  override suspend fun getAlbumByTrackId(
    trackId: String,
  ...
  override suspend fun getAlbumsByTrackIds(trackIds: List<String>): List<CoreTrackAlbum> {
    ...
    val networkTracks =
      if (stillNeedDataTrackIds.isNotEmpty()) {
        network.getSeveralTracks(stillNeedDataTrackIds).successOrNothing {
  ...
  override suspend fun getAlbumBySpotifyId(
    albumId: String,
    shouldQueryNetwork: Boolean,
  ): FullAlbum? {
