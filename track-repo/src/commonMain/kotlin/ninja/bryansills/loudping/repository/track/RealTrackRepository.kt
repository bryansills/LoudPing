...
private fun NetworkTrackAlbum.toDatabase(): ninja.bryansills.loudping.core.model.TrackAlbum =
  ninja.bryansills.loudping.core.model.TrackAlbum(
    spotifyId = this.id,
    title = this.name,
...
private fun SimplifiedArtist.toDatabase(): Artist = Artist(spotifyId = this.id, name = this.name)
...
private fun Map<String, Track?>.toMultiTrackResult(): MultiTrackResult {
  ...
  return if (missingIds.isEmpty()) {
    ...
    MultiTrackResult.Mixed(tracks = foundTracks, missingIds = missingIds)
  }
}
