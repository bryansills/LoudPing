...
class RealDatabaseService(
  ...
  private val timestampFormatter: DateTimeFormat<DateTimeComponents> =
  ...
  override suspend fun insertTrackPlayGap(start: Instant, end: Instant) {
    ...
    queries.insert(start = start.format(timestampFormatter), end = end.format(timestampFormatter))
  }
  ...
  override suspend fun getTrackForSpotifyId(trackId: String): Track? =
    try {
      val tracks =
        database.trackQueries.get_track_from_spotify_id(spotifyTrackId = trackId).awaitAsList()
      tracks
        ...
        .map { rowsForTrack ->
          ...
          Track(
            ...
            artists =
              rowsForTrack.map { trackArtist ->
                Artist(spotifyId = trackArtist.spotify_artist_id, name = trackArtist.artist_name)
              },
          )
        }
        .firstOrNull()
  ...
  override suspend fun getTracksForSpotifyIds(trackIds: List<String>): List<Track> {
    val tracks =
      database.trackQueries.get_tracks_from_spotify_ids(spotifyTrackIds = trackIds).awaitAsList()
  ...
  override suspend fun getAlbumForSpotifyId(albumId: String): FullAlbum? =
    try {
      val databaseResults =
        database.albumQueries.get_album_from_spotify_id(spotifyAlbumId = albumId).awaitAsList()
  ...
  override suspend fun insertTrack(track: Track) {
    database.transaction { insertTrackInternal(track = track) }
  }

  override fun getTracksBetween(newerTime: Instant, olderTime: Instant) = flow {
    val pageTimestamps =
      database.trackPlayRecordQueries
    ...
    (listOf(olderTime) + pageInstants + newerTime).windowed(2).forEach { (older, newer) ->
      val pageResults =
        database.trackPlayRecordQueries
            ...
            TrackPlayRecord(
              track =
                Track(
                  ...
                  artists =
                    rowsForRecord.map { dbRow ->
                      Artist(spotifyId = dbRow.spotify_artist_id, name = dbRow.artist_name)
                    },
                ),
              timestamp = Instant.parse(firstRow.timestamp),
  ...
  private suspend fun insertTrackInternal(track: Track) {
    ...
    insertAlbumInternal(album = track.album, associatedTrackIds = listOf(track.spotifyId))
    track.artists.forEach { trackArtist ->
      insertArtistInternal(artist = trackArtist, associatedTrackIds = listOf(track.spotifyId))
    }
  }

  private suspend fun insertArtistInternal(artist: Artist, associatedTrackIds: List<String>) {
    database.artistQueries.insert_artist(spotifyId = artist.spotifyId, name = artist.name)
    associatedTrackIds.forEach { trackId ->
