...
class TransformableKeyedQueryPagingSource<Key : Any, DatabaseRowType : Any, RowType : Any>(
    ...
    protected var currentQuery: Query<DatabaseRowType>? by
                ...
                val getPagingSourceLoadResult: TransactionCallbacks.() -> LoadResult<Key, RowType> =
                    {
                        ...
                        val databaseResults =
                            queryProvider(key, nextKey).also { currentQuery = it }.executeAsList()

                        val results = transform(databaseResults)

                        LoadResult.Page(data = results, prevKey = previousKey, nextKey = nextKey)
