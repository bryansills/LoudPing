...
class RealAuthManager(
  ...
  private fun getFullState(timestamp: Instant): String =
    "$timestamp$authorizeUrlSalt".toRandomString()

  override fun getAuthorizeUrl(startTime: Instant): String {
    val result =
      networkSneak.authorizeUrl
        .httpUrlBuilder {
          addQueryParameter("response_type", "code")
          addQueryParameter("client_id", networkSneak.clientId)
          addQueryParameter(
            "scope",
            "user-read-recently-played user-library-read user-read-private",
          )
  ...
  override suspend fun setAuthorizationCode(
    state: String,
    code: String,
    startTime: Instant,
  ): String {
    ...
    val apiResponse =
      spotifyAuthService.requestTokens(
  ...
  override suspend fun setRefreshToken(refreshToken: String): String = getValidAccessTokenInternal {
    refreshToken
  }
  ...
  private suspend fun getValidAccessTokenInternal(
    ...
    val currentAccessTokenExpiresAt =
      Instant.parse(simpleStorage.first(Stored.AccessTokenExpiresAt))
  ...
  override val rawValues: Flow<RawAuthValues> =
    combine(
...
private inline fun String.httpUrlBuilder(block: HttpUrl.Builder.() -> Unit): HttpUrl =
  this.toHttpUrl().newBuilder().apply { block() }.build()

private fun String.toRandomString(length: Int = 16): String {
  val alphabet: List<Char> = ('a'..'z') + ('A'..'Z') + ('0'..'9')
  val randomInstance = Random(this.hashCode())
  return (1..length).map { alphabet.random(randomInstance) }.joinToString("")
}
