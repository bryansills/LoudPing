...
fun main() {
  ...
  mainScope.launchBlocking {
    ...
    //                        println("Cached count: ${event.found.size} missing count:
    // ${event.missing.size}")
    //                    }
    //                    chunksCached++
    //                }
...
private fun processData(records: List<DeepHistoryRecord>) {
  ...
  val playCount =
    records.groupBy(
      ...
      valueTransform = {
        "${it.master_metadata_album_artist_name} - ${it.master_metadata_track_name}"
      },
    )

  val top100 = playCount.values.sortedByDescending { it.count() }.take(100)

  top100.forEach { songData -> println("Song: ${songData.first()} Played: ${songData.count()}") }
