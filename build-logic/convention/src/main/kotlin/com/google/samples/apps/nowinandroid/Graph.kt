 ...
 * - [Graph.invoke] is **recursively** searching through dependent projects (although in practice it
 *   will never reach a stack overflow).
 * - [Graph.invoke] is entirely re-executed for all projects, without re-using intermediate values.
 * - [Graph.invoke] is always executed during Gradle's Configuration phase (but takes in general
 *   less than 1 ms for a project).
 *
...
private abstract class GraphDumpTask : DefaultTask() {
  ...
  private fun mermaid() = buildString {
    ...
    rootProjects
      .flatMap { it.value }
      .sortedDescending()
      .forEach { appendLine(it.alias(indent = 2, plugins.get().getValue(it))) }
    // Links
